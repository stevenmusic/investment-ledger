...
- 台股：證交所 MIS 即時報價 `mis.twse.com.tw`、歷史走勢 `www.twse.com.tw/exchangeReport/STOCK_DAY`
- 美股：Yahoo Finance chart API、圖表用 TradingView 嵌入式 widget
- 匯率：`api.frankfurter.app`
- 圖表元件：Chart.js（從 cdnjs.cloudflare.com 載入，畫資產成長/資產配置/台股走勢）

---

## 目前範圍

✅ 總覽（總投入/總市值/總損益/總報酬率、資產成長曲線、漲跌幅前五、資產配置圖）
✅ 持倉管理（表格化：搜尋/篩選市場/點表頭排序，網頁表單新增/刪除，本機儲存、不需登入）
✅ 交易紀錄（買入/賣出/股息/手續費，本機儲存）
✅ 觀察清單（自訂目標價到價提醒）
✅ 個股查詢：美股用 TradingView 完整圖表(含均線/MACD/RSI)＋基本資料；
   台股因為 TradingView 免費圖表沒有資料授權，改用證交所公開資料自畫近一月走勢
✅ 分析（最佳/最差標的、已實現交易勝率、月度已實現損益 — 需要先在交易紀錄填「賣出」並填「原成本」）
✅ 快捷操作列（新增交易／新增股票／匯入CSV）
✅ 跌幅提醒（5% / 10% / 15%，鎖定建立時成本，方案A；打開App自動檢查）
✅ 選用：背景推播通知（即使沒開App也跳通知，需要一次性GitHub登入）
✅ 資料備份：設定分頁可以匯出/匯入整份備份（JSON檔）
✅ 每人各自 Fork 一份，資料互相獨立
✅ 視覺風格：Dark Clean Professional（深色冷色調，參考 TradingView / Binance）

## CSV 匯入持倉格式

第一列要有欄位名稱（順序不重要，欄位名稱要小寫）：

```csv
market,ticker,name,shares,avgCost
TW,2330,台積電,1000,600
US,AAPL,Apple,20,180
```

## 關於台股圖表的限制

TradingView 的免費嵌入式圖表元件，資料是另外跟各交易所買的授權，目前不包含台灣證交所（TWSE），
所以即使換各種代號格式都一樣會出現「此商品僅在TradingView上可用」——這是 TradingView 自己的資料授權範圍問題，
不是任何網站設定錯誤就能解決的。美股因為有完整授權，所以一切正常。

台股目前改用證交所公開的 `STOCK_DAY` 開放資料自己畫近一個月收盤價走勢，沒有均線/MACD/RSI這些技術指標
（要自己刻指標計算，目前先不做）。如果證交所那個 API 之後格式改了或不能用了，這部分可能需要再調整。

## 之後可以加（先不做）

- 全英文介面切換按鈕
- 持倉表格在手機上改成卡片排列（目前是橫向滾動表格）
- 台股技術指標（均線/MACD/RSI 自己算）
- 股息追蹤獨立報表（目前股息只記錄在交易紀錄裡，沒有單獨統計）
- 真正的時間加權報酬率（目前的「報酬率」是簡單的（現值-成本）/成本，沒有處理資金進出時間點）
- 跨裝置同步（目前資料只在單一裝置的瀏覽器裡）
